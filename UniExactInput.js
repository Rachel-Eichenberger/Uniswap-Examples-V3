...
async function ExactIn(TokenSelling,TokenBuying,Fee,AmountSending,SlipP){
	// get deadline
	...
	var QuoterEnc = {'tokenIn':TokenSelling,'tokenOut':TokenBuying,'fee':Fee.toString(),'recipient':FROM,'deadline':Deadline.toString(),'amountIn':AmountSending.toString(),'amountOutMinimum':'0','sqrtPriceLimitX96':'0'};
	...
	var myData2 = MySwap.methods.exactInputSingle(encoded).encodeABI();
	
	

	var gas = await MySwap.methods.exactInputSingle(encoded).estimateGas({from:FROM,data:myData2,to:V3Swapper}).catch(function(error){concole.log(error);});
...
async function RUN(){
	...
	await ExactIn(WETH,USDC,3000,"1000000000000000000","50"); // weth -> usdc swap 0.3% pool 1 ETH 0.5% slippage
}

RUN()
//ExactInETH(WETH,USDC,3000,"1000000000000000000","50");